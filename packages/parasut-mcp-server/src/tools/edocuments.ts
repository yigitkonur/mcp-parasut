...
import { formatSuccess, formatNotFound, type ToolResponse } from '../utils/response.js';
...
export const edocumentTools: Tool[] = [
  {
...
export async function handleCheckEInvoiceInbox(args: unknown): Promise<ToolResponse> {
  try {
...
export async function handleSendEInvoice(args: unknown): Promise<ToolResponse> {
  try {
    ...
    if (!hasFirstConfirmation || !hasSecondConfirmation) {
      ...
      if (!hasFirstConfirmation) missingConfirmations.push('confirm=true');
      if (!hasSecondConfirmation) missingConfirmations.push('i_understand_this_is_irreversible="YES"');
...
export async function handleSendEArchive(args: unknown): Promise<ToolResponse> {
  try {
...
export async function handleSendESmm(args: unknown): Promise<ToolResponse> {
  try {

...
import { type ServerConfig, loadConfig, validateConfig } from './config.js';
 ...
 */
async function readBodyWithLimit(req: IncomingMessage, maxBytes = 2 * 1024 * 1024): Promise<string> {
  let body = '';
  let bytesRead = 0;
  ...
  const server = http.createServer(async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (url.pathname === '/oauth/register') {
      ...
      if (effectiveApiKey) {
        ...
        if (!regToken || !safeTimingEqual(regToken, effectiveApiKey)) {
          ...
          res.setHeader(
            'WWW-Authenticate',
            `Bearer error="invalid_token", error_description="Dynamic Client Registration requires a valid Initial Access Token", resource_metadata="${metadataUrl}"`
          );
    ...
    if (url.pathname === '/oauth/token') {
      ...
      if (req.headers.authorization?.startsWith('Basic ')) {
        try {
          ...
          if (u) params['client_id'] = u;
          if (p) params['client_secret'] = p;
        } catch {
      ...
      try {
        ...
        if (grantType === 'client_credentials') {
          ...
          const token = oauthServer.issueClientCredentialsToken(
            clientId,
    ...
    if (authRequired) {
      const isValid = Boolean(
        bearerToken && (
          (effectiveApiKey && safeTimingEqual(bearerToken, effectiveApiKey)) ||
      ...
      if (!isValid) {
        ...
        res.setHeader(
          'WWW-Authenticate',
          `Bearer error="invalid_token", error_description="Missing or invalid Bearer token", resource_metadata="${metadataUrl}"`
        );
        res.writeHead(401, { 'Content-Type': 'application/json' });
        res.end(
          JSON.stringify({
      ...
      if (!isValid) {
        ...
        res.setHeader(
          'WWW-Authenticate',
          `Bearer error="invalid_token", error_description="Invalid Bearer token", resource_metadata="${metadataUrl}"`
        );
        res.writeHead(401, { 'Content-Type': 'application/json' });
        res.end(
          JSON.stringify({

...
import { formatSuccess, formatList, formatCreated, formatNotFound, formatContactSummary, } from '../utils/response.js';
...
export async function handleSearchContacts(args) {
    try {
        ...
        if (params.account_type)
            filter['account_type'] = params.account_type;
        if (params.city)
...
export async function handleGetContact(args) {
    try {
        ...
        const client = getClient();
        const include = ['category', 'contact_portal'];
...
export async function handleUpdateContact(args) {
    try {
        ...
        if (params.tax_number !== undefined)
            attributes['tax_number'] = params.tax_number;
        if (params.tax_office !== undefined)
            attributes['tax_office'] = params.tax_office;
        if (params.city !== undefined)
            attributes['city'] = params.city;
        if (params.district !== undefined)
            attributes['district'] = params.district;

...
import { formatSuccess, formatList, formatCreated } from '../utils/response.js';
...
export async function handleSearchTransactions(args) {
    try {
        ...
        return formatList(transactions, {
            totalCount: response.meta?.total_count ?? transactions.length,
...
export async function handleCreateBankFee(args) {
    try {
...
export async function handleGetFinancialSummary(_args) {
    try {
        const client = getClient();
